...
<script type="text/javascript">
	//改变商品的数量
	function changeCount(id, count, pnum) {//id是商品的id，count,是要改变的数量.
		//需要将count转换成数字
...
<body>
	...
	<c:if test="${not empty cart}">
		...
		<table border='1' align='center' width="65%">
			...
			<c:forEach items="${cart}" var="c">
				<tr>
					<td><input type="checkbox" name="delCk" value="${c.key.id}"></td>
					<td>${c.key.name }</td>
					<td>${c.key.price }</td>
					<td>
						<input type="button" value="-" onclick="changeCount('${c.key.id}','${c.value-1}')">
						<input type="text" value="${c.value}" size="7" style="text-align:center"
	...
	<a href="http://localhost:8080">回首页继续购物</a>
</body>
</html>
